...
Page(Object.assign({}, quantity, Zan.Toast,{
	...
	getGoods (){   					//获取商品列表信息
		let {goods,active,ase,category} = this.data;
		var _this = this,
			hasData = false;

		//先判断该商品内容下是否有数据
		for(var i in goods){
			if(goods[i].cid == active.id){
				hasData = true;
				break;
			}
		}
	...
	getMoreGoods (e){				//加载更多数据
		var _this= this;
		let {goods,active,loading,ase} = this.data;
	...
	setGoodQuan (newgoods){					//设置每个商品的点击加减
	...
	orderNow (){			//立即下单
		let {shopcart,MODE,billId} = this.data,_this = this;
    ...
    let totalPrice = this.data.totalPrice
		http.request({
			...
			if(res.errorCode == 200){
				if(MODE == "SHOP"){			//shop模式下新增到清单列表中，而不直接下订单
	...
	toggleDialog() {			//打开购物车
		...
		if(Object.keys(shopcart).length<=0){return false;}
		this.setData({
	...
	searchGoods(){
		...
		if(utils.trim(searchTxt).length<=0){return false;}
		app.handle(() => {
			this.setData({
				showSearch:true
			});
